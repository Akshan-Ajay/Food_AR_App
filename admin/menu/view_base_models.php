...
/* ---------- FETCH BASE MODELS ---------- */
$sql = "SELECT * FROM CategoryARModels ORDER BY Category ASC";
...
<head>
    ...
    <title>Base AR Models — Carrie's Cafe</title>
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-accent:hover { background: #d35400; transform: translateY(-2px); }
...
<div class="main">
    <div class="topbar">
        <span class="page-title">Base AR Models</span>
    ...
    <div class="content">
        <div class="card">
            <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:15px; margin-bottom: 25px; border-bottom: 1px solid var(--border); padding-bottom: 20px;">
                <h2 style="font-family:'Playfair Display', serif;">📦 Model Overview</h2>
                <div style="display:flex; gap:10px;">
                    <a href="add_customization.php" class="btn btn-accent">➕ Add Customization</a>
                    <a href="View_Customizations.php" class="btn btn-primary">⚙️ Custom Data</a>
                </div>
            </div>
            ...
            <div class="table-container">
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="font-weight: 600; color: var(--sidebar);"><?php echo htmlspecialchars($row['Category']); ?></td>
                            <td><span style="color: #888; font-size: 0.8rem;">LKR</span> <?php echo number_format($row['BasePrice'], 2); ?></td>
                            
                            <td>
                                <?php if(!empty($row['ModelGLB'])): ?>
                            ...
                            <td style="text-align: right;">
                                <div style="display:inline-flex; gap:8px;">
                                    <a href="edit_base_model.php?category=<?php echo urlencode($row['Category']); ?>" class="btn btn-primary" style="padding: 6px 12px;">
                                        ✏️ Edit
                                    </a>
                                    <a href="delete_base_model.php?category=<?php echo urlencode($row['Category']); ?>" 
                                       class="btn btn-danger" 
                                       style="padding: 6px 12px;"
                                       onclick="return confirm('Are you sure you want to delete the base model for <?php echo $row['Category']; ?>?');">
                                        🗑️
                                    </a>
                                </div>
                            </td>
